...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
        super(client, handler, {
            command: 'elementnumber',
            aliases: ['enumber'],
            description: 'Gives you the info of the given element. ',
            category: 'educative',
            usage: `${client.config.prefix}elementnumber [number]`,
            baseXp: 30
        })
    }
    
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        if (!joined) return void M.reply('Give me an element number, Baka!')
        const chitoge = joined.trim()
        await axios.get(`https://neelpatel05.pythonanywhere.com/element/atomicnumber?atomicnumber=${chitoge}`)
        .then((response) => {
